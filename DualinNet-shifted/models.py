...
class DualinBlock(layers.Layer):
    def __init__(self, feature_num, kernel_size, my_initial, my_regular):
        ...
        self.D1 = DBlock(feature_num, kernel_size, my_initial, my_regular)
        self.D2 = DBlock(feature_num, kernel_size, my_initial, my_regular)
    ...
    def call(self, input_main, input_aux):
        ...
        main_out = self.D1(input_1st) + input_main

        input_1st = tf.concat([main_out, input_aux],3)
        aux_out = self.D2(input_1st) + input_aux

        return main_out, aux_out


class DualinNet_shifted(tf.keras.Model):
    def __init__(self):
        super(DualinNet_shifted, self).__init__()
...
class DualinNet_64_20(tf.keras.Model):
    def __init__(self):
        super(DualinNet_64_20, self).__init__()
        self.my_initial = tf.random_normal_initializer(0, 0.01)
        ...
        self.conv2 = layers.Conv2D(64, (3,3), padding = 'SAME',
                              kernel_initializer = self.my_initial, kernel_regularizer = self.my_regular)
        self.dualblock_iter = [DualinBlock(64,(3,3),self.my_initial, self.my_regular) for i in range(20)]
    ...
    def call(self, input):
        ...
        for l in self.dualblock_iter:
            main_20, aux_20 = l(wav_input, pixel_input)
...
class DualinNet_96_5(tf.keras.Model):
    def __init__(self):
        super(DualinNet_96_5, self).__init__()
        self.my_initial = tf.random_normal_initializer(0, 0.01)
    ...
    def call(self, input):
        ...
        combined = tf.concat([main_5, aux_5],3)
