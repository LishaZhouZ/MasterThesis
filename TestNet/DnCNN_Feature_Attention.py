from model_utility import ConvBlock, ConvConcatLayer, WaveletConvLayer, WaveletInvLayer,ConvBlock10
...
class KaiNet(tf.keras.Model):
    ...
    def call(self, input):
        ...
        post_dwt= self.wavelet(input)
        
        LL = self.conv10a(post_dwt)
        HL = self.conv10b(post_dwt)
        LH = self.conv10c(post_dwt)
        HH = self.conv10d(post_dwt)

        combined = tf.concat([LL, LH, HL, HH], 3)
        post_idwt = self.invwavelet(combined)
        output = input + post_idwt
        return output